...
select * from metodopago; -- descripcionMetodo / pkIdMetodoPago
SELECT * FROM proveedor; -- direccionProv / pkIdProv
SELECT * FROM empleado; -- nombreEmpleado * apellidoEmpleado / pkIdEmpleado
SELECT * FROM empresa; -- nombreEmpresa / pkIdEmpresa
SELECT * FROM sucursal; -- nombreSucursal / pkIdSucursal
SELECT * FROM bodega; -- descripcionBodega / pkIdBodega
SELECT * FROM tipocompra; -- nombretipocompra / pktipocompra

SELECT P.direccionProv AS Proveedor, E.apellidoEmpleado AS Empleado, EM.nombreEmpresa, SUC.nombreSucursal, BOD.descripcionBodega, CE.fechaCompra, CE.totalCompra, TC.nombretipocompra, MP.descripcionMetodo
...
P.pkIdProv = CE.fkIdProv AND E.pkIdEmpleado = CE.fkIdEmpleado AND EM.pkIdEmpresa = CE.fkIdEmpresa 
AND SUC.pkIdSucursal = CE.fkIdSucursal AND BOD.pkIdBodega = CE.fkIdBodegadestino AND TC.pktipocompra = CE.fktipocompra AND MP.pkIdMetodoPago = CE.fkmetodoPago
AND MP.pkIdMetodoPago = 1;

-- compras que cierto empleado realizó
SELECT P.direccionProv AS Proveedor, E.apellidoEmpleado AS Empleado, EM.nombreEmpresa, SUC.nombreSucursal, BOD.descripcionBodega, CE.fechaCompra, CE.totalCompra, TC.nombretipocompra, MP.descripcionMetodo
FROM proveedor P, empleado E, empresa EM, sucursal SUC, bodega BOD, compraencabezado CE, tipocompra TC, metodopago MP WHERE
P.pkIdProv = CE.fkIdProv AND E.pkIdEmpleado = CE.fkIdEmpleado AND EM.pkIdEmpresa = CE.fkIdEmpresa 
AND SUC.pkIdSucursal = CE.fkIdSucursal AND BOD.pkIdBodega = CE.fkIdBodegadestino AND TC.pktipocompra = CE.fktipocompra AND MP.pkIdMetodoPago = CE.fkmetodoPago
AND E.pkIdEmpleado = 1;

-- Quién realizó la compra
SELECT CE.pkNoDocumentoEnca, CE.totalCompra, CE.fechaCompra, P.direccionProv AS Proveedor, E.apellidoEmpleado, E.nombreEmpleado
FROM proveedor P, empleado E, empresa EM, sucursal SUC, bodega BOD, compraencabezado CE, tipocompra TC, metodopago MP WHERE
P.pkIdProv = CE.fkIdProv AND E.pkIdEmpleado = CE.fkIdEmpleado AND EM.pkIdEmpresa = CE.fkIdEmpresa 
AND SUC.pkIdSucursal = CE.fkIdSucursal AND BOD.pkIdBodega = CE.fkIdBodegadestino AND TC.pktipocompra = CE.fktipocompra AND MP.pkIdMetodoPago = CE.fkmetodoPago
AND CE.pkNoDocumentoEnca= 2;
...
fechaCompra BETWEEN '4/2/2021' AND '5/5/2021';
